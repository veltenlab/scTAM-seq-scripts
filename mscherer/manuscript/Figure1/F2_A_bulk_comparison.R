...
plot <- ggplot(to_plot, aes_string(x='BulkMethylation', y='Methylation', color='Cluster'))+geom_point()+geom_smooth(method='lm',se=FALSE)+xlim(0, 1)+ylim(0,1)+
  ...
  geom_text(aes(label=paste('Pearsons R²:', format(Correlation, digits=2))), x=0.25, y=0.9, check_overlap=TRUE, color='black')+
  plot_theme+
  scale_color_manual(values=color_map)+ylab('Pseudo-bulk methylation')+xlab('Bulk methylation')
ggsave(file.path(plot_path, 'F2_A_bulk_vs_singlecell.pdf'), plot, width=150, height=100, units='mm')
...
plot <- ggplot(to_plot, aes_string(x='BulkMethylation', y='Methylation'))+geom_point(aes(color=ifelse(Type=='Imprinted', '5', Cluster)))+geom_smooth(aes(color=ifelse(Type=='Imprinted', '6', Cluster)), method='lm',se=FALSE)+xlim(0, 1)+ylim(0,1)+
  ...
  plot_theme+
  scale_color_manual(values=color_map)+ylab('Pseudo-bulk methylation')+xlab('Bulk methylation')
ggsave(file.path(plot_path, 'F2_A_bulk_vs_singlecell_imprinted.pdf'), plot, width=150, height=100, units='mm')
