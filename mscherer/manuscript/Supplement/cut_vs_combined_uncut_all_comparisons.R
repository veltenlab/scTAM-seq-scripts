...
to_plot <- c()
dat_uncut <- read.table(all_uncut[1], 
                        sep="\t", 
                        header=T)
doublets_uncut <- read.csv(uncut_doublet[1])
...
dat_uncut2 <- read.table(all_uncut[2], 
                        sep="\t", 
                        header=T)
doublets_uncut2 <- read.csv(uncut_doublet[2])
doublets_uncut2 <- c(doublets_uncut2$Barcode[doublets_uncut2$DoubletDetectionLabel==1])
dat_uncut2 <- dat_uncut2[!(row.names(dat_uncut2)%in%doublets_uncut2), ]
names_uncut1 <- setdiff(row.names(dat_uncut), row.names(dat_uncut2))
names_uncut2 <- setdiff(row.names(dat_uncut2), row.names(dat_uncut))
dat_uncut <- as.data.frame(rbind(dat_uncut[names_uncut1, ],
                                 dat_uncut2[names_uncut2, ]))
...
plot <- ggplot(to_plot, aes(x=Uncut, y=Cut, color=Type))+
  geom_point()+geom_abline(slope=1, intercept=0)+facet_wrap(Type~Comparison, ncol = 4)+
...
ggsave(file.path(plot.path, 'F1_B_cut_vs_combined_uncut_all.pdf'), plot, width=175, height=100, units='mm')
...
to_plot$Type <- factor(to_plot$Type, levels=c("No HhaI cutsite",
                                              ...
                                              "Differential CpG Bcells"))
plot <- ggplot(to_plot, aes(x=Uncut, y=Cut, color=Type))+
  geom_point(size=.75)+geom_abline(slope=1, intercept=0, size=.25)+facet_wrap(Comparison~Type, ncol = 4)+
...
ggsave(file.path(plot.path, 'F1_B_cut_vs_combined_uncut_selected.pdf'), plot, width=200, height=200, units='mm')
