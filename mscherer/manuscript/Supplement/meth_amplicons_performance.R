...
ampli.info <- read.csv('/users/lvelten/project/Methylome/infos/BCells/amplicons_CpG_GC_annotated.csv', row.names=1)
...
ggsave('/users/lvelten/project/Methylome/analysis/scTAMseq_manuscript/Supplement/amplicon_performance_GCContent.pdf',
       width=100, height=75, unit='mm')

plot <- ggplot(ampli.info, aes(x=Type, y=CpG_count, fill=Type))+geom_boxplot()+plot_theme+
  scale_fill_manual(values=colors_amplicons)
ggsave('/users/lvelten/project/Methylome/analysis/scTAMseq_manuscript/Supplement/amplicon_performance_CpG_count.pdf',
       width=100, height=75, unit='mm')

dat <- read.table('/users/lvelten/project/Methylome/analysis/missionbio/re_sequencing/Sample7_70_percent_good_performance/tsv/Sample7_70_percent_good_performance.barcode.cell.distribution.tsv', header=TRUE)
rowinfo <- read.csv('/users/lvelten/project/Methylome/analysis/missionbio/re_sequencing/Sample7_70_percent_good_performance/tsv/rowinfo.csv',
                    row.names=1)
...
ggsave('/users/lvelten/project/Methylome/analysis/scTAMseq_manuscript/Supplement/dropout_GCContent_methylated.pdf',
       width=100, height=75, unit='mm')
...
ggsave('/users/lvelten/project/Methylome/analysis/scTAMseq_manuscript/Supplement/dropout_CpG_count_methylated.pdf',
       width=100, height=75, unit='mm')
...
plot <- ggplot(to_plot, aes(x=GC_Content, y=Dropout, color=Type))+geom_point()+geom_abline(slope=1, intercept = 0)+plot_theme+
  xlim(0, 1)+ylim(0,1)+scale_color_manual(values=colors_amplicons)
ggsave('/users/lvelten/project/Methylome/analysis/scTAMseq_manuscript/Supplement/dropout_GCContent_nonHhaI.pdf',
       width=100, height=75, unit='mm')
