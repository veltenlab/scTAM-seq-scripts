...
dat_cut <- read.table(paste0("../../data/", cut, "/tsv/", cut, ".barcode.cell.distribution.tsv"), 
                      sep="\t", 
                      header=T)
rowinfo <- read.csv(paste0('../../misc/', cut, '/tsv/rowinfo.csv'),
                    row.names = 1)
dat_uncut <- read.table(paste0("../../data/", uncut, "/tsv/", uncut, ".barcode.cell.distribution.tsv"), 
                        sep="\t", 
                        header=T)
