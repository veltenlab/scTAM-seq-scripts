...
means_clusters <- read.csv('/users/mscherer/cluster/project/Methylome/analysis/dropou_modeling/BM/all_amplicons.csv', row.names=1)
colinfo <- read.table("/users/mscherer/cluster/project/Methylome/infos/BCells/CpGs.value.per.amplicon.Blood.Bone.marrow.complete.array.data.txt", header=T,
                      row.names=8)
special <- read.csv("/users/mscherer/cluster/project/Methylome/analysis/scTAMseq_manuscript/revision/colinfo_BM.csv",
                    row.names=1)
