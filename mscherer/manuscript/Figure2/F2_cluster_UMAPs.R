...
# Based on the bulk data (differential CpGs)
library(Seurat)
color_map <- list(CellType=c('naive'='#fcbd7e',
                             'memory1'='#fc6571',
                             'memory2'='#fc3262'),
...
more_info <- more_info[!is.na(more_info$background.cpgs), ]
row.names(more_info) <- more_info$amplicon
common.names <- intersect(row.names(more_info), colnames(filtered.counts))
more_info <- more_info[common.names, ]
filtered.counts <- filtered.counts[, common.names]
meth.data.numeric <- meth.data.numeric[more_info$background.cpgs, ]
p.vals <- apply(meth.data.numeric, 1, function(x){
  t.test(x[cell_assignment$V2%in%'csMBC'],
              x[cell_assignment$V2%in%'ncsMBC'])$p.value
})
sel_amplis <- common.names[p.vals<0.01]
sel_data <- filtered.counts[row.names(rowinfo[grepl('Cluster2', rowinfo$CellType_reclustering), ]), sel_amplis]
seurat.obj <- CreateSeuratObject(t(sel_data),
                                 assay = "DNAm",
                                 meta.data = rowinfo)
seurat.obj <- NormalizeData(seurat.obj,
...
seurat.obj <- FindClusters(seurat.obj, resolution = 0.5)
seurat.obj <- RunUMAP(seurat.obj, dims = 1:10)
plot <- DimPlot(seurat.obj, reduction = 'umap', group.by = 'CellType_reclustering')+plot_theme
ggsave(paste0(plot_path, 'UMAP_MBCs_diff_CpGs.pdf'), plot)
