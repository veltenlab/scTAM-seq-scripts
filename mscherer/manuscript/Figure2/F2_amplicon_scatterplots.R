...
library(viridis)
library(GGally)
plot_path <- '/users/mscherer/cluster/project/Methylome/analysis/scTAMseq_manuscript/Figure2/Sample8/amplicon_correlation/'
...
selected_amplicons <- row.names(more_info[match(names(which(selected_amplicons)), more_info$background.cpgs), ])
...
selected <- ifelse(filtered.counts[row.names(rowinfo), row.names(amplicon.info)]>0, 1, 0)

for(clust in unique(cell_metadata$Cluster)){
#  selected_amplicons <- cluster_meth[, clust]>0.3&cluster_meth[, clust]<0.7
  vars <- apply(selected[cell_metadata$Cluster%in%clust, ], 2, var)
#  selected_amplicons <- rank(vars)>(length(vars)-30)|rank(vars)<10
  ...
  pheatmap(selected_data, 
                 ...
                 fontsize=15)
  dev.off()
  cors <- cor(selected[cell_metadata$Cluster%in%clust, selected_amplicons])
  corrplot::corrplot(cors, diag=FALSE, type='upper')
  
  for_mutual <- selected[cell_metadata$Cluster%in%clust, selected_amplicons]
  not_wanted <- c(which(apply(for_mutual, 2, function(x)all(x==0))),  which(apply(for_mutual, 2, function(x)all(x==1))))
  for_mutual <- for_mutual[, !(colnames(for_mutual)%in%names(not_wanted))]
  ...
  corrplot::corrplot(na.omit(infos),
                     tl.col = "black",
                     is.corr = FALSE,
                     type = 'upper',
