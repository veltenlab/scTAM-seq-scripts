#### F2_X_protein_analysis.R ###################################
#' This file is used to generate plots associated with the protein data of scTAM-seq
#' 

library(ggplot2)
plot_path <- '/users/mscherer/cluster/project/Methylome/analysis/scTAMseq_manuscript/Figure2/Sample8/protein/'
...
dat <- read.csv('/users/mscherer/cluster/project/Methylome/analysis/missionbio/re_sequencing/Sample8_70_percent_good_performance/tsv/rowinfo.csv',
                    row.names = 1)
...
frac.q1 <- format(sum(grepl('memory B-cells', sel_dat[q1, 'CellType']))/sum(q1), digit=2)
frac.q2 <- format(sum(grepl('memory B-cells', sel_dat[q2, 'CellType']))/sum(q2), digit=2)
frac.q3 <- format(sum(grepl('memory B-cells', sel_dat[q3, 'CellType']))/sum(q3), digit=2)
frac.q4 <- format(sum(grepl('memory B-cells', sel_dat[q4, 'CellType']))/sum(q4), digit=2)
plot <- ggplot(sel_dat, aes(x=CD27, y=CD25, color=CellType))+geom_point(size=.05)+geom_vline(xintercept = thres.cd27)+geom_hline(yintercept = thres.cd25)+
...
ggsave(file.path(plot_path, 'antibody_scatterplots_CD27_CD25.pdf'), plot,  width=40, height=40, unit='mm')
...
plot <- ggplot(sel_dat, aes(x=CD27, y=CD71, color=CellType))+geom_point(size=.05)+geom_vline(xintercept = thres.cd27)+geom_hline(yintercept = thres.cd71)+
  ...
  xlim(-1.5, 6.1)+
  scale_color_manual(values=color_map)+plot_scatter
ggsave(file.path(plot_path, 'antibody_scatterplots_CD27_CD71.pdf'), plot,  width=40, height=40, unit='mm')
...
plot <- ggplot(sel_dat, aes(x=CD1c, y=CD71, color=CellType))+geom_point(size=.05)+geom_vline(xintercept = thres.cd1c)+geom_hline(yintercept = thres.cd71)+
  ...
  xlim(-1.5, 6.1)+
  scale_color_manual(values=color_map)+plot_scatter
ggsave(file.path(plot_path, 'antibody_scatterplots_CD1c_CD71_memory_Bcells.pdf'), plot,  width=40, height=40, unit='mm')
...
plot <- ggplot(sel_dat, aes(x=CD27, y=CD71, color=CellType))+geom_point(size=.05)+geom_vline(xintercept = thres.cd27)+geom_hline(yintercept = thres.cd71)+
  ...
  xlim(-1.5, 6.1)+
  scale_color_manual(values=color_map)+plot_scatter
ggsave(file.path(plot_path, 'antibody_scatterplots_CD27_CD71_positive.pdf'), plot,  width=40, height=40, unit='mm')
