...
sel.amplicons <- sel.amplicons[allelic_dropout<0.75&allelic_dropout>=0.25]
...
vals_naive <- apply(selected[cell_metadata$CellType%in%'naive B-cells', ], 2, function(ampli){
  c(sum(ampli==0),
    sum(ampli>0),
    sum(ampli>0)/length(ampli))
})
vals_memory <- apply(selected[cell_metadata$CellType%in%'memory B-cells', ], 2, function(ampli){
  c(sum(ampli==0),
    sum(ampli>0),
    sum(ampli>0)/length(ampli))
})
